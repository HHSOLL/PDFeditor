...
- External replacement readiness: **80 / 100 verified**
...
repair, compare report generation, and batch automation are available from the
product inspector and save through the engine into real PDF structure. A
generated 100-PDF corpus validates qpdf-backed open and structure behavior, and
a local release package smoke verifies that built assets and the product engine
server can run outside the Vite development server. The remaining blocker for an
external 90+ claim is not feature labels; it is missing broad manual
...
| Image / vector / object editing | 8 | 6 | Engine-backed image delete/replace/move tests plus UI move path that uses `moveImage` instead of overlay-only movement | Image crop/rotate/resize UI, vector style editing, object inspector smoke |
...
| Compare / batch automation | 4 | 3 | Inspector compare target selection, compare report PDF download, batch watermark/search-redaction/sanitizer quick action, production-server compare/batch smoke, 100-job runner | Saved action builder, queue/worker execution, interactive diff overlay |
...
- Compare and batch automation are no longer engine-only. The inspector can
  ...
  and run a structural batch quick action that applies watermark, search
  redaction, and sanitizer steps through the engine.
...
The shortest path from the current verified 80 to a claimable 90 is:
...
5. Add interactive object inspector, image crop/rotate/resize UI, saved batch
   action builder, and interactive compare overlay.
