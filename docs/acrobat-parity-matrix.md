...
| Native image/vector/object editing | 8 | 7 | Existing image detection plus engine-backed delete/replace/move path removes or redacts the source image area; UI source-image reposition emits `moveImage`; vector drawings are extracted, surfaced in the document overlay, shown in the object inspector, and `deleteVector` removes touched colored line art through PDF redaction graphics removal | `tests/engine-image-object-edit.mjs`, `tests/engine-image-move.mjs`, `tests/engine-vector-object-edit.mjs`, source image coverage inside `tests/engine-smoke.mjs`, image move/vector UI regressions in `tests/pdf-editor.spec.ts` | Existing images can be selected/moved/resized/rotated/cropped/deleted/replaced; image data and vector paths are actually removed or rewritten | Add image crop/rotate UI polish, vector move/color/stroke editing, richer object inspector, image raw-byte corpus |
...
- **Native vector editing:** vector detection/delete and a basic object inspector are UI-accessible; vector movement, color/stroke editing, and rich object-property editing are not complete.
- **Compare:** current compare has JSON/report PDF output, changed-area regions, inspector-triggered report download, and document overlays, but not annotation/object diff.
- **Batch automation:** current batch support is a validated 100-job engine manifest runner plus inspector quick action and a persisted preset, not a full action-builder UI or queued production worker.
